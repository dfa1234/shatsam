...
Copyright (c) 2014 Vladimir Agafonkin - BSD 2-Clause "Simplified" License
https://github.com/mourner/suncalc

Copyright (c) 2021 Stuart Casarotto - MIT License
https://github.com/SCasarotto/nanoleaf-virtual-skylight
...
const sunTimesMatrix: [
  number,
  keyof GetSunTimesResult,
  keyof GetSunTimesResult
][] = [
...
export const addTime = (
  angle: number,
  riseName: keyof GetSunTimesResult,
  setName: keyof GetSunTimesResult
): void => {
  sunTimesMatrix.push([angle, riseName, setName]);
};
...
export const getSunTimes = (
  ...
  } as GetSunTimesResult;

  for (let i = 0; i < sunTimesMatrix.length; i += 1) {
    const sunTime = sunTimesMatrix[i];
    const h0 = (sunTime[0] + dh) * rad;

    const Jset = getSetJ(h0, lw, phi, dec, n, M, L);
    const Jrise = Jnoon - (Jset - Jnoon);

    result[sunTime[1]] = fromJulian(Jrise);
    result[sunTime[2]] = fromJulian(Jset);
  }
...
export const getMoonIllumination = (date: Date): GetMoonIlluminationResult => {
...
export const getMoonTimes = (
  ...
  const result: GetMoonTimes = {} as GetMoonTimes;
